...
function generateBabyName(name, gender, childId) {
	...
	if (!!name && name !== "Unnamed") {
		return name.replace(/[^a-zA-Z\u4e00-\u9fa5 ]+/g, "").substring(0, 30);
	}
	let names = [];
	switch (gender) {
		case "m":
			// eslint-disable-next-line prettier/prettier
			names = ['艾迪森','阿尔杰农','艾伦','阿尔法','安东','阿克塞尔','巴扎','本顿','伯纳德','布兰德','布雷特','卡尔','卡尔文','卡罗尔','扎克','查克里','克莱尔','康奈利厄','克罗夫顿','达尔登','达克斯','丹','德文','迪格里','唐','道格拉斯','德里斯科尔','杜安','达克','爱德蒙','埃尔斯登','弗里曼','加比','加兰','乔治','戈弗雷','格雷姆','格里尔','哈蒙德','哈兰','亨德里克斯','赫里曼','休伊','休','伊迪纳','因格拉姆','杰基','贾斯珀','杰克逊','杰科布','杰尔','坎丁','卡尔西','肯达尔','凯文','里恩','基兰','柯里','兰尼','劳森','拉兹','利兰达','利维尔','林登','林顿','利奥尔','朗尼','卢卡斯','曼利','马瑞卡','梅尔林','迈克尔','蒙蒂','墨菲','内特','内德','诺维尔','奥代尔','奥利','奥伯特','奥托','佩吉特','派普','昆廷','雷蒙德','里奇','罗伯特','罗斯','鲁道夫','萨米','斯科蒂','斯塔西','萨德','西奥多','汤米','特里','泰森','瓦尔','弗农','威利斯','威尔默','温顿','温斯顿'];
			break;
		case "f":
			// eslint-disable-next-line prettier/prettier
			names = ['阿德琳','艾琳','艾列莎','艾莉雅','艾莉森','安洁莉卡','安娜莉丝','安诺拉','阿沙妮亚','贝茜','贝琪','贝蒂','毕蒂','布莉安娜','卡梅利亚','卡密尔','卡穆琳','卡洛琳','查丝提蒂','切尔西','切尔茜','辛迪','克莱马蒂丝','达拉','黛','黛比','德尔法','爱莉诺拉','爱莉安娜','伊丽莎白','伊莉丝','艾默生','埃米琳','埃莉卡','艾莉','尤丝塔西娅','伊芙琳','加布莉埃尔','乔治安娜','哈珀','哈里埃塔','海莉','阿兹','亨特','海信丝','印蒂安娜','茵蒂','杰凯塔','詹妮','詹宁','詹可','凯特琳','凯姆','柯露','科琳','库琳','考特妮','克丽丝黛','拉薇娜','琳恩','莱拉','莱丝蕾娅','琳茜','洛蕾娜','露西尔','路薇妮娅','琳','莱萨','玛德琳','玛丽安','马蒂','莫琳','玛可辛','梅乐迪','米拉妮','米丝蒂','奈特','诺艾尔','奥托琳','佩格','珀琳','佩顿','珀尔','佩尔莉','彼得罗内尔','菲比','珀茜','普蕾丝','蕾扎娜','赛琳娜','谢尔琳','莎尔拉','肖娜','丝凯','丝贝拉','特蕾茜','特蕾莎','特鲁迪','沃丽丝','薇尔达','伦温','伊薇特'];
			break;
	}
	// eslint-disable-next-line prettier/prettier
...
function recordSperm({
	...
	if (typeof spermOwner === "string" || spermOwner instanceof String) {
		...
		if (!spermOwner.role) {
			...
			spermOwnerName = spermOwner.role.replace("panty_thief","内裤小偷").replace("school_nurse","学校护工").replace("tower_creature","塔楼生物").replace("steed","坐骑").replace("pinch","林奇").replace("black_dog","黑狗").replace("bishop_right_hand","右护法").replace("bishop_left_hand","左护法").replace("bishop","主教").replace("nurse","护士").replace("waitress","服务员").replace("waiter","服务员").replace("scientist","科学家").replace("inmate","犯人").replace("farm_tower_guard","瞭望塔守卫").replace("guard","守卫").replace("hacker","黑客").replace("woman","女人").replace("man","男人").replace("islander","岛民").replace("pirate","海盗") + "——" + spermOwner.name;
		} else {
