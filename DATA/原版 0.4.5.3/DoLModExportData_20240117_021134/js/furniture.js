...
const Furniture = (() => {
	...
	function furnitureInit() {
		...
		mapper.chairs.set('name', {
			...
			description: "一张用来坐的凳子。",			Description for the shop interface to show.
		...
		*/
		...
		mapper.set("chair", {
			...
			type: ["starter"],
			cost: 0,
			description: "一把破旧、嘎吱作响的椅子。晃且坐着难受。",
		...
		mapper.set("egg", {
			...
			type: ["comfy"],
			cost: 2420,
			description: "一张颜色奇异的圆形扶手椅，非常难组装。",
		...
		mapper.set("desk", {
			...
			type: ["stable", "starter"],
			cost: 0,
			description: "一张老旧的书桌，刻满了过往孤儿留下的涂鸦。",
			iconFile: "desk.png",
		});
		...
		mapper.set("deskMidcentury", {
			...
			type: ["stable"],
			cost: 1550,
			description: "一张现代主义风格的书桌，在二十世纪中叶十分流行。",
		...
		mapper.set("singlebeddeluxe", {
			...
			type: ["single", "comfy"],
			cost: 2400,
			description: "一张符合人体工学设计的单人床，非常舒适。",
		...
		mapper.set("doublebedexotic", {
			...
			type: ["double", "comfy"],
			cost: 4884,
			description: "一张现代极简主义风格的床。非常舒适，可以容下两个人。",
		...
		mapper.set("doublebedwicker", {
			...
			type: ["double", "comfy"],
			cost: 4860,
			description: "一张全藤织框架的双人床。非常舒适，可以容下两个人。",
		...
		mapper.set("carved", {
			...
			type: ["organiser"],
			cost: 4620,
			description: "一个纯手工雕刻的豪华衣橱，含有多个抽屉和衣架杆。",
