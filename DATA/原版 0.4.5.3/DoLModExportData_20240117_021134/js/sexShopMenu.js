...
setup.sextoys = [
	...
	},
	{
		...
		description: "用于肛门调教，可以塞进肛门或直接玩弄。",
		cost: 8000,
		type: ["dildo", "anal"],
		category: "butt_plug",
		wearable: 1,
	...
	},
	{
		...
		description: "该物品产生的震动能让人产生强烈的快感，很适合刚开始接触性玩具的人。",
		cost: 12000,
		wearable: 0,
		size: 0,
		category: "vibrator",
	...
	},
	{
		...
		description: "一包三片装的催情剂。在性爱前'适量'服用会增强性爱体验。",
		cost: 4000,
		wearable: 0,
		size: 3,
...
function sexShopOnItemClick(index) {
	...
	for (const index in item.colour_options) {
		coloringDiv +=
			(index === 0
				? `<br><span style="color: #e0e0e0">可选颜色: </span><div id="ssm_colour_panel" class="colour-options-div"><br> `
				: "") +
			`
			<div colour-name="${item.colour_options[index]}" onclick="window.sexShopOnColourClick(\`${item.colour_options[index]}\`)" class="colour-button div-link">
				<div class="bg-${item.colour_options[index]}">
					<span class="capitalize colour-name-span">${setup.colourName(item.colour_options[index].replace("-"," "))}</span>
					<a tabindex="0"></a>
				</div>
			</div>`;
	}
	...
	<div class="ssm_desc_border">
		...
		(V.money >= item.cost
			? `<a id="ssmBuyButton" onclick="window.sexShopOnBuyClick(${item.index})" class="ssm_buy_button">
			  ...
			  item.cost / 100 +
			  `</span>)`
			: `<span class="ssm_not_enough_money">钱不够</span> (<span class="gold">£${item.cost / 100}</span>)`) +
...
function determineRecipient(index) {
	...
	for (const li of ["Alex", "Eden", "Kylar", "Robin", "Sydney"]) {
		if (isLoveInterest(li)) {
			optionBuilder += `<option value="${li}">${li.replace("None","没有人").replace("Robin","罗宾").replace("Whitney","惠特尼").replace("Kylar","凯拉尔").replace("Eden","伊甸").replace("Avery","艾弗里").replace("Sydney","悉尼").replace("Alex","艾利克斯").replace("Black Wolf","黑狼").replace("Great Hawk","巨鹰")}</option>`;
		}
	}
...
function sexShopOnGiftClick(index) {
	...
	document.getElementById("ssmGiftButton").outerHTML = `<span class="ssm_gift_button ssm_fade_in" id="ssmGiftButton" style="color:#97de97">已买！</span>`;
...
function sexShopOnBuyClick(index, inSexShop = true, colour, costsMoney = true) {
	...
	if (inSexShop) {
		...
		document
			...
			.getElementsByClassName("ssm_already_owned")[0].innerHTML = `<span class="ssm_owned_text ssm_fade_in">已拥有</span>`;
		/* fade in/out bought green text indicator */
		document.getElementById("ssmBuyButton").outerHTML = `<span class="ssm_buy_button ssm_fade_in" id="ssmBuyButton" style="color:#97de97">已买！</span>`;
				...
				if (document.getElementById("ssmBuyButton"))
					document.getElementById("ssmBuyButton").outerHTML =
						V.money > item.cost
							...
							: `<span class="ssm_not_enough_money">钱不够</span>`;
				sexShopOnBuyClick.counter = "off";
			}, 1400);
		}
	}
