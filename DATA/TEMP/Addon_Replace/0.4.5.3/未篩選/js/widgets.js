...
function genderappearancecheck() {
	...
	addfemininityfromfactor(T.femininity_multiplier * 200, "体型");
	...
	addfemininityfromfactor(T.femininity_multiplier * 100 * T.acting_multiplier, "姿势(x" + T.acting_multiplier + "因语文技能而产生的效果)");
	...
	if (V.worn.over_upper.exposed >= 2 && V.worn.upper.exposed >= 2) {
		...
		if (V.worn.under_upper.exposed >= 1) {
			...
			addfemininityfromfactor((V.player.perceived_breastsize - 0.5) * 100, (V.player.perceived_breastsize > 0 ? "露出的乳房" : "露出的钢板"));
		} else {
